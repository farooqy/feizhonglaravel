...
class accountController extends Controller
{
	...
	public function companyLogin(Request $request)
	{
		...
		{
			$this->setError(["The company phone and password do not match ".Hash::make($request->company_password)]);
			return $this->error;
		}
		...
		{
			$this->setError(["The company password and phone do not match ".Hash::check($data[0]->comp_pass, $request->company_password)]);
			return $this->error;
		}
    ...
    public function basicInfoRegister(Request $request)
    {
    	...
    	else if($verification_code->count() > 1)
    	{
    		...
    		phoneVerificationModel::where('target_phone', $request->company_phone)->delete();
