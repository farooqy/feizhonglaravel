<template>
  <article
    class="hentry blog-post blog-post-v3"
  ...
  >
    <div class="post__author author vcard inline-items" style="margin-left: 25px;">
      ...
      <div class="author-date">
        ...
        posted
        <a :href="getProductLink()" @click.prevent="viewProduct()">product</a>
        <div class="post__date">
          <time class="published" :datetime="created_at">{{" at "+created_at}}</time>
        </div>
      </div>
    </div>
    <div style="display: flex;margin-bottom: 15px;padding-left: 25px;" class="row">
      ...
      <div class="post-content xs-12">
        <a href="#" @click.prevent="viewProduct()" class="h3 post-title">{{product_name}}</a>
        <p v-html="makeLineText(product_description)"></p>

        <div class="post-additional-info inline-items">
          <div class="names-people-likes">
            <a href="#" class="post-add-icon inline-items"></a>
            <b>
              <u>{{ product_currency }} {{product_price }} / {{product_unit}}</u>
            </b>
          </div>
    ...
    <ul class="comments-list" v-for="(comment,ckey) in in_comments" v-bind:key="ckey">
      <li class="comment-item" :class="hasChildren(comment)" style="background-color: aliceblue;">
        <div class="post__author author vcard inline-items">
          ...
          <div class="author-date">
            <a class="h6 post__author-name fn" href="#">{{getCommentName(comment)}}</a>
            <div class="post__date">
              <time class="published" datetime="2017-03-24T18:18">{{comment.created_at}}</time>
            </div>
          </div>
        ...
        <ul class="children">
          ...
          >
            <div class="post__author author vcard inline-items">
              ...
              <div class="author-date">
                <a class="h6 post__author-name fn" href="#">{{getCommentName(reply)}}</a>
                <div class="post__date">
                  <time class="published" datetime="2017-03-24T18:18">{{reply.created_at}}</time>
                </div>
              </div>
            </div>
    ...
    <form class="comment-form inline-items" v-if="isLoggedIn()">
      ...
      <button
        :class="requestLoading ? 'btn btn-md-2 btn-secondary' : 'btn btn-md-2 btn-primary'"
  ...
  <article class="hentry post" v-else>
    <div class="post__author author vcard inline-items">
      ...
      <div class="author-date">
        ...
        >{{uploaded_by_name}}</a>
        posted
        <a :href="getStatusLink()" @click.prevent="viewStatus()">status</a>
        <div class="post__date">
          <time class="published" :datetime="status_time">{{status_time}}</time>
        </div>
      </div>
    ...
    <p v-html="makeLineText(status_text)"></p>
...
module.exports = {
  data: function() {
    return {
      ...
      post_token:
        this.product_token === undefined
          ? this.status_generated_token
          : this.product_token,
      comment_reply: null,
      show_reply_box: -1,
  ...
  methods: {
    ...
    submitComment(type, comment = null) {
      ...
      var comment_text;
      if (this.in_comment_text === null && this.comment_reply === null) return;
      else if (type === "comment" && comment === null) return;
        ...
        this.post_id =
          this.status_id === undefined ? this.product_id : this.status_id;
        this.post_token =
          this.status_generated_token === undefined
    ...
    getCommentProfile(comment) {
      if (comment.host_type === "comp") return comment.comp_profile.comp_logo;
      else return comment.person_profile.user_profile;
    },
    getCommentName(comment) {
      if (comment.host_type === "comp") return comment.comp_profile.comp_name;
      else
        return (
          comment.person_profile.user_fname +
          " " +
